...
- **Cuentas semilla del mock login neutralizadas**: la migración 014 creó usuarios reales (`admin@example.com`, `seller1@example.com`, `seller2@example.com`) con contraseña conocida (`password123`). La migración `20260724100500` les asigna una contraseña aleatoria irrecuperable, cerrando ese backdoor **sin borrar las cuentas** (los proyectos creados durante la fase de demo tienen `owner_id` de esos perfiles y un borrado en cascada los destruiría). Cuando esos proyectos se reasignen a cuentas reales (`UPDATE projects SET owner_id = ...`), las cuentas semilla pueden eliminarse definitivamente.
