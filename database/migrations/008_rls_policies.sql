...
-- ════════════════════════════════════════════
-- § 1.5  LOOKUP TABLES (read-only for all, write for admin)
-- ════════════════════════════════════════════

-- component_types
CREATE POLICY component_types_admin_all
  ON component_types FOR ALL TO authenticated
  USING (get_user_role() = 'admin')
  WITH CHECK (get_user_role() = 'admin');

CREATE POLICY component_types_read
  ON component_types FOR SELECT TO authenticated
  USING (true);

-- connection_types
CREATE POLICY connection_types_admin_all
  ON connection_types FOR ALL TO authenticated
  USING (get_user_role() = 'admin')
  WITH CHECK (get_user_role() = 'admin');

CREATE POLICY connection_types_read
  ON connection_types FOR SELECT TO authenticated
  USING (true);
...
-- orientations
CREATE POLICY orientations_admin_all
  ON orientations FOR ALL TO authenticated
  USING (get_user_role() = 'admin')
  WITH CHECK (get_user_role() = 'admin');

CREATE POLICY orientations_read
  ON orientations FOR SELECT TO authenticated
  USING (true);

-- product_types
CREATE POLICY product_types_admin_all
...
-- transport_types
CREATE POLICY transport_types_admin_all
  ON transport_types FOR ALL TO authenticated
  USING (get_user_role() = 'admin')
  WITH CHECK (get_user_role() = 'admin');

CREATE POLICY transport_types_read
  ON transport_types FOR SELECT TO authenticated
  USING (true);
